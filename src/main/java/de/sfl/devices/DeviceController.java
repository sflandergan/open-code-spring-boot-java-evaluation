...
public class DeviceController {
	...
	public ResponseEntity<DeviceDto> createDevice(@Valid @RequestBody CreateDeviceDto dto) {
		var createdDevice = deviceService.createDevice(dto.toEntity());
		return ResponseEntity.status(HttpStatus.CREATED).body(DeviceDto.fromEntity(createdDevice));
	}
	...
	public ResponseEntity<DeviceDto> updateDevice(@PathVariable UUID deviceId, @Valid @RequestBody UpdateDeviceDto dto) {
		var updatedDevice = deviceService.updateDevice(deviceId, dto.name(), dto.description());
	...
	@Operation(summary = "Assign a sensor to a device")
	@ApiResponses(value = {
	...
	public ResponseEntity<DeviceDto> assignSensor(@PathVariable UUID deviceId, @PathVariable Long sensorId) {
		var updatedDevice = deviceService.assignSensor(deviceId, sensorId);
		return ResponseEntity.ok(DeviceDto.fromEntity(updatedDevice));
	}
}
