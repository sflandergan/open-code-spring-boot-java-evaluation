...
public class DeviceController {
	...
	public ResponseEntity<DeviceDto> updateDevice(@PathVariable UUID id, @Valid @RequestBody UpdateDeviceDto updateDeviceDto) {
		var updatedDevice = deviceService.updateDevice(id, updateDeviceDto);
		return ResponseEntity.ok(DeviceDto.from(updatedDevice));
	}

	@PutMapping("/{deviceId}/sensors/{sensorId}")
	@Operation(summary = "Assign sensor to device", description = "Assigns a sensor to a device idempotently")
	@ApiResponses(value = {
	...
	@DeleteMapping("/{id}")
	@Operation(summary = "Delete device", description = "Deletes a device and all of its sensor assignments")
	@ApiResponses(value = {
